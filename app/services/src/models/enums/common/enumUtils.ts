 ...
 *
 * @template T
 * @param {string} stringValue - The string value to be converted to an enum value.
 ...
 */
const castStringToEnum = <T>(stringValue: string, enumValues: T[], defaultValue: T): T => (
  (enumValues.includes(stringValue as T)) ? stringValue as T : defaultValue
);
