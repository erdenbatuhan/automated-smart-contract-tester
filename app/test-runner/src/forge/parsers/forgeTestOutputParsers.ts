import type ForgeUnprocessedTestResults from '@forge/types/ForgeUnprocTestResults';
import type ForgeTestOutput from '@forge/types/ForgeTestOutput';
import type { ForgeTestResult } from '@forge/types/ForgeTestOutput';
...
const getTestResultsFromForgeExecutionResults = (
  ...
  .flatMap(([contract, { test_results }]) => Object.entries(test_results || [])
    .map(([test, { status, reason, decoded_logs, kind }]) => ({
      contract: forgeCommonParsers.parseContractName(contract),
      test: forgeCommonParsers.parseTestName(test),
      status,
      reason,
